...
from dead_by_dawn_sim.persona_objectives import closeout_adjustment, objective_adjustment
...
def score_action(
    ...
    score = _base_action_score(choice, state, ruleset, persona, actor, target)
    score += objective_adjustment(choice, state, persona)
    score += closeout_adjustment(choice, state, ruleset, persona)
    score += _actor_state_adjustment(actor, ruleset, persona)
    return score
...
def _base_action_score(
    ...
    if choice.action_id in {"advance", "fall_back"}:
        return _movement_action_score(choice, state, ruleset, persona, actor)
    ...
    score = sum(persona.weights.get(tag, 0.0) for tag in action.tags)
    if target.hp <= max(2, target.max_hp // 3):
        score += persona.weights.get("finisher", 0.0)
    score += _context_adjustment(choice, state, ruleset, persona, actor, target)
    if choice.push:
...
def _actor_state_adjustment(actor: ActorState, ruleset: Ruleset, persona: ScoredPersona) -> float:
    adjustment = 0.0
    if actor.hp <= max(2, actor.max_hp // 3):
        adjustment += persona.weights.get("low_hp_modifier", 0.0)
    if actor.stress >= ruleset.core.stress.panic_threshold:
        adjustment += persona.weights.get("high_stress_modifier", 0.0)
    return adjustment
...
def _context_adjustment(
    ...
    action = ruleset.actions[choice.action_id]
    adjustment = 0.0
    adjustment += _heal_context_adjustment(action.tags, target, persona)
    adjustment += _stress_context_adjustment(action.tags, target, ruleset, persona)
    adjustment += _control_context_adjustment(action.tags, target, persona)
    adjustment += _grit_context_adjustment(choice, actor, persona)
    adjustment += _rally_context_adjustment(choice, actor, target, persona)
    adjustment += _trip_context_adjustment(choice, state, actor, target, persona)
    adjustment += _shove_context_adjustment(choice, actor, target, persona)
...
def _heal_context_adjustment(
    action_tags: list[str], target: ActorState, persona: ScoredPersona
) -> float:
    if "heal" not in action_tags:
        return 0.0
    missing_hp = target.max_hp - target.hp
    adjustment = -4.0 if missing_hp == 0 else missing_hp * persona.weights.get("heal_urgency", 0.2)
...
def _stress_context_adjustment(
    action_tags: list[str], target: ActorState, ruleset: Ruleset, persona: ScoredPersona
) -> float:
    if "stress" not in action_tags:
        return 0.0
    headroom = max(0, ruleset.core.stress.panic_threshold - target.stress)
    return headroom * persona.weights.get("stress_setup", 0.15)
...
def _grit_context_adjustment(
    choice: ActionChoice, actor: ActorState, persona: ScoredPersona
) -> float:
    ...
    if not in_real_danger:
        return -persona.weights.get("grit_topoff_penalty", 3.5)
    return persona.weights.get("grit_danger_bonus", 0.8)


def _rally_context_adjustment(
    choice: ActionChoice, actor: ActorState, target: ActorState, persona: ScoredPersona
) -> float:
...
def _push_adjustment(actor: ActorState, ruleset: Ruleset, persona: ScoredPersona) -> float:
